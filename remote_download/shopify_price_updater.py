                        ...
                        try:
                            if index >= index_start:
                                ...
                                if length and length >= 50:
                                    self.merchant_helper.product_update_price_batch(merchant_info_list, group_num)
                                    merchant_info_list = []
                                    self._init_update_log_record(group_num, str(product_id), index+1, size, update=True)
                                    self._check_running_expiration_date()
                        except Exception as e:
                            error_note = str(e)
                            self._init_update_log_record(group_num, str(product_id), index+1, size, error_note)
                            raise TypeError("Something Wrong")
                        else:
                            if index == size - 1 and merchant_info_list:
                                self.merchant_helper.product_update_price_batch(merchant_info_list, group_num)
                                self._init_update_log_record(group_num, str(product_id), size, size, update=True)
